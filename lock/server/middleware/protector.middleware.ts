...
const dataProtectionMiddleware = handleAsync(async (req, res, next) => {
  try {
    ...
    const passwords = await Password.findOne().sort({ creationTimestamp: -1 }).lean<IPassword[]>();
    let dataProtect = undefined;
    if (!passwords) {
      console.info("Password is gonna be generated")
      dataProtect = await generatePassword(dataProtector);
    }else{
      (req as any).passwordAddress = passwords[0].address
    }
